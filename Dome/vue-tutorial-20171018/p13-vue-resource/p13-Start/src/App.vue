...
export default {
    ...
    methods: {
        submit() {
            ...
            // this.$http.post('https://vue-resource-tutorial-2514e.firebaseio.com/users.json', this.user)
            // .then(response => {         // define how to deal with the response
            //     console.log(response);
            // }, error => {               // define how to deal with error
            //     console.log(error);
        ...
        fetchData() {
            this.$http.get('')
                .then(response => {
                    return response.json();      // this prints the promise object NOT the data object
                }, error => {
                    console.log(error);
                }).then(data => {
                    const resultArray = [];
